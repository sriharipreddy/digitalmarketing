...
import { StubIntelligenceDriver } from './_services/intelligence.driver.js';
import { CompetitorService } from './_services/competitor.service.js';
import { AutopilotService } from './_services/autopilot.service.js';
...
async function bootstrap(): Promise<void> {
  ...
  const driver = new StubIntelligenceDriver();
  logger.info({ driver: driver.constructor.name }, 'intelligence_driver_ready');

  const competitorService = new CompetitorService(models, driver);
  const autopilotService = new AutopilotService(sequelize, models);

  const competitorController = new CompetitorController(competitorService);
  const autopilotController = new AutopilotController(autopilotService);
  ...
  app.use('/api/v1/intelligence', createApiRouter({ competitorController, autopilotController }));
