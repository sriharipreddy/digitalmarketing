...
import { StubDiscoveryDriver } from './_services/discovery.driver.js';
import { InfluencerService } from './_services/influencer.service.js';
import { OutreachService } from './_services/outreach.service.js';
...
async function bootstrap(): Promise<void> {
  ...
  const discoveryDriver = new StubDiscoveryDriver();
  logger.info({ driver: discoveryDriver.constructor.name }, 'discovery_driver_ready');

  const influencerService = new InfluencerService(models, discoveryDriver);
  const outreachService = new OutreachService(models, influencerService);
  ...
  app.use('/api/v1/influencer', createApiRouter({ influencerController, outreachController }));
